...
/**
 * Clean rebuild of the single‑page fitting app.
 *
 * Participants can:
 *  - Choose a model (Linear, Log‑linear, Emax-inhibitory)
 *  - Adjust parameters via sliders
 *  - See curve update with data and live RSS
 *
 * Data: synthetic Emax with doses [0.5, 1, 2, 3] mg/kg, constructed so ~20% remains at 3 mg/kg.
 * Equation text for the selected model is shown above the sliders.
 */

// ---------- Synthetic data (Emax) ----------
// Model used to generate: y = E0 - (Emax * d^h) / (ED50^h + d^h)
// Chosen params -> at d=3, y≈20: E0=100, Emax=80, ED50=0.01, h=1
...
// ---------- Models ----------
const LOG_OFFSET = 1e-3; // fixed small constant for log-linear
...
const models = {
  ...
  "Log-linear": {
    ...
    params: {
      ...
      // note: no offset slider; LOG_OFFSET is fixed constant
    },
...
export default function App() {
  ...
  const curve = useMemo(() => {
    const xs = Array.from({ length: 321 }, (_, i) => i * 0.01); // 0..3.2
    return xs.map((x) => ({ x, y: clamp01(models[modelKey].fn(x, params)) }));
  }, [params, modelKey]);

  // predictions at demo doses
  const preds = useMemo(() => demoData.map((d) => models[modelKey].fn(d.x, params)), [params, modelKey]);
  const rssVal = useMemo(() => rss(demoData.map((d) => d.y), preds), [preds]);
    ...
    <div className="min-h-screen w-full bg-white text-gray-900">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Controls */}
          <div key={modelKey} className="md:col-span-1 bg-gray-50 rounded-2xl p-4 shadow-sm border border-gray-200">
            <div className="mb-4">
            ...
            <div className="h-[420px]">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={curve} margin={{ top: 10, right: 20, bottom: 10, left: 0 }}>
                  ...
                  />
                  <YAxis dataKey="y" domain={[0, 120]} ticks={[0,20,40,60,80,100,120]} label={{ value: "% remaining", angle: -90, position: "insideLeft" }} />
